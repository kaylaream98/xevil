...
// "sound.cpp"  X11/UNIX sound engine implementation.
...
// in sound_cmn.h -- ensure_init() asserts the count matches SOUND_MAX.
static const char *const SOUND_FILES[] = {
  ...
  "terraexm.mp3",     // 37 TERRAEXM_SOUNDTRACK
  "newsong.mp3",      // 38 NEWSONG_SOUNDTRACK
};

#define SOUND_FILES_NUM ((int)(sizeof(SOUND_FILES) / sizeof(SOUND_FILES[0])))

// The streaming soundtracks available to the X11 random rotation: the full set
// of MIDI-derived tracks.
...
void SoundManager::ensure_init() {
  ...
  // The filename table is indexed by SoundName, so it must line up with the
  // enum in sound_cmn.h.
  assert(SOUND_FILES_NUM == SoundNames::SOUND_MAX);
  ...
  if (initRes != MA_SUCCESS) {
    ...
    // audio.  This is the path that runs on a machine with no sound device.
    delete m_impl;
    m_impl = NULL;
    m_engineOk = False;
