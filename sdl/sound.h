...
// "sound.h"
// X11 sound engine for XEvil 2.5.
//
// The UNIX build was silent from 1994 until this rewrite; the original manual
// invited "some industrious soul" to add sound.  This is that engine.  It is
// backed by miniaudio (see x11/vendor/miniaudio.h) but that dependency is kept
// entirely inside sound.cpp -- this header exposes the same public API shape as
// the old stub / the Win32 SoundManager so cmn/ code compiles unchanged.  The
// audio device is opened LAZILY on first use; if no device is available (e.g. a
// headless CI machine) the manager marks itself disabled forever and every
// method becomes a cheap no-op that never blocks or crashes.
